...
## 第十六轮：下拉菜单修复 + 汇率行增删 + 更多组件自定义（2026-08-29）

- **下拉弹出层"透明框"修复**：`_TightComboBox` 的弹出菜单改用小半径清晰阴影（blur 12 / offset 2）替代默认 30px 大模糊，并把视口边距对称化。坑：在菜单子类 `__init__` 里 `setShadowEffect` 会使 view 的 GC 状态异常、随后 `exec(aniType=...)` 解析失败——调整必须在 `_createComboMenu` 返回前完成（已注释说明）。
- **汇率行可增删**：组件底部新增 +/− 按钮（最多 6 行、至少 1 行，达限时自动禁用）；配置语义为"已配置槽位"，删除时后续配置前移保持行连续；渲染时未配置的槽位回落默认货币对。
- **更多组件自定义**：时钟新增 12 小时制开关（h:mm AP 格式）；天气新增温度单位选择（°C/°F/K，切换即时重渲染不重新联网）；待办新增「x/y 已完成」计数（可关）。
- 设置窗口信息显示页同步渲染新选项；86 passed。
