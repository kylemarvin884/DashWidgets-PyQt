...
|| 组件 | 描述 | 功能 |
||------|------|------|
|| 🕐 **时钟** | 时间日期显示 | 实时时钟，无边框纯文字样式 |
|| 📊 **系统监控** | 硬件状态监测 | CPU、内存、磁盘使用率实时显示 |
|| 📡 **网络监控** | 网络状态 | 实时网速、磁盘使用率显示 |
|| ⏱️ **秒表** | 正计时工具 | 精确计时 |
|| ⏲️ **计时器** | 倒计时工具 | 预设/自定义时长倒计时，归零提醒 |
|| 🍅 **番茄钟** | 专注工作 | 25 分钟工作 + 休息循环 |
|| ⏳ **倒数日** | 重要日子提醒 | 距目标日期的倒计天数，点击设置 |
|| 📅 **日历** | 日期显示 | 当月日历，今日高亮 |
|| 🔋 **电池** | 电量状态 | 环形电量指示，充电/剩余时间（笔记本） |
|| 📝 **笔记** | 快速记录工具 | 桌面便签，防抖自动保存 |
|| ✅ **待办** | 任务管理 | 待办清单，支持添加删除标记完成 |
|| 🎵 **音乐** | 媒体控制 | 控制系统媒体播放，显示当前曲目 |
|| 🌤️ **天气** | 天气信息 | 实时天气（Open-Meteo），IP 自动定位，后台异步加载 |
|| 💱 **汇率** | 汇率查询 | 央行参考汇率（frankfurter.app），USD/EUR/GBP/JPY/HKD |
|| 📰 **RSS 订阅** | 资讯聚合 | 多源订阅合并展示，点击打开原文 |
|| 🖼️ **图片** | 自定义图片展示 | 大图自动降采样，等比适配 |
|| 📄 **文档查看器** | 文档阅读 | 查看 PDF、Word、Markdown、TXT |
|| 🤖 **自动化** | 自动化示例 | 插件自动化能力演示 |
